...
export const EMAIL_ALREADY_EXISTS_MSG = "An account with this email already exists.";
...
export const signUpEmailValidator = z.string().email({ message: "Please enter a valid email" }).max(SIGNUP_MAX_EMAIL_LENGTH, {
  message: `Email must be no more than ${SIGNUP_MAX_EMAIL_LENGTH} characters long`,
});
export const signUpPasswordValidator = z
  .string()
  .min(SIGNUP_MIN_PASSWORD_LENGTH, { message: `Password must be at least ${SIGNUP_MIN_PASSWORD_LENGTH} characters long` })
  .max(SIGNUP_MAX_PASSWORD_LENGTH, { message: `Password must be no more than ${SIGNUP_MAX_PASSWORD_LENGTH} characters long` })
  .regex(passwordRegex, {
    message:
